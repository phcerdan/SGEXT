...
int main(int argc, char* const argv[]) {
  ...
  opt_desc.add_options()("exportSDP,e", po::value<std::string>(),
                         "Export the resulting set of points in a simple "
                         "(sequence of discrete point (sdp)).");
  opt_desc.add_options()(
      "exportGraph,g", po::value<std::string>(),
      "Export the resulting set of points as a graph. It saves a list of nodes "
      "(.nod) and a list of edges (.edg)");
  bool parseOK = true;
  po::variables_map vm;
  ...
  if(!parseOK || vm.count("help") || argc <= 1) {
    trace.info()
        << "Compute the thinning of a volume using an AsymetricThinningScheme"
        << endl
        << endl
        << "Basic usage: " << endl
  ...
  constantValue = (invert_image == true) ? itk::NumericTraits<PixelType>::max()
                                         : itk::NumericTraits<PixelType>::min();
  ItkImageType::SizeType padRegion;
  for(unsigned int i = 0; i < Dim; ++i) padRegion[i] = 1;
  ...
  using Complex = DGtal::VoxelComplex<KSpace, Object>;
  ...
  if(sk == "ulti")
    ...
    Skel = oneIsthmus<Complex>;
  else if(sk == "is")
    Skel = skelIsthmus<Complex>;
  else if(sk == "1isthmus")
  ...
  if(sel == "random")
    ...
    Select = [&dt](const Complex::Clique& clique) {
  ...
  if(perfect) {
    ...
    do {
      ...
      for(DigitalSet::Iterator it = S.begin(); it != S.end(); ++it) {
        ...
        nb++;
        if(dt(*it) <= layer) {
          if(shape.isSimple(*it)) Q.push(it);
        }
      }
  ...
  // THEN( "visualize the cells" )
  {
    ...
    if(vm.count("exportGraph")) {
      ...
      for(auto&& p = verts.first; p != verts.second; ++p) {
        ...
        // adjacency_iterator; std::pair<adjacency_iterator,adjacency_iterator>
        // vp1 = boost::adjacent_vertices( *p, graph );
      };
      ...
      std::cout << "#Verts perfect: " << boost::num_vertices(shape)
                << std::endl;

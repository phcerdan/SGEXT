...
int main(int argc, char *const argv[]) {
    ...
    opt_desc.add_options()(
            "useGraphviz,g", po::bool_switch()->default_value(false),
    ...
    opt_desc.add_options()("outputFolder,o",
                           ...
                           "Output folder. The filename would be based on the "
                           "input, but the suffix "
                           "would be .vtp");
    ...
    if (useGraphviz) { // read graphviz
        sg = SG::read_graphviz_sg(filenameGraph);
    } else {
    ...
    writer->Write();
    if (verbose) {
        std::cout << "Output vtp file: " << output_full_path.string()
                  << std::endl;
    }

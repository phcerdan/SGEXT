...
TEST_F(ParticleGraphGlueData_Fixture, works) {
    ...
    EXPECT_EQ(
            std::size(
                    particle_graph_data.connected_list.collection[0].neighbors),
            1);
    EXPECT_EQ(
            std::size(
                    particle_graph_data.connected_list.collection[1].neighbors),
            3);
    EXPECT_EQ(
            std::size(
                    particle_graph_data.connected_list.collection[2].neighbors),
            1);
    EXPECT_EQ(
            std::size(
                    particle_graph_data.connected_list.collection[3].neighbors),
            1);
    auto &neighbors =
            particle_graph_data.connected_list.collection[1].neighbors;

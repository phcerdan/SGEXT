...
// Unit testing is done in the external project, this just checks that works
// here.
TEST(visualize_histograms, test1) {
    std::vector<double> data{0.0, 1.0, 1.0, 1.0, 2.0, 3.0,
                             5.0, 5.0, 8.0, 8.0, 12.0};
    histo::Histo<double> h(
            data, histo::GenerateBreaksFromRangeAndBins<double>(0.0, 15.0, 5));
...
TEST(visualize_histograms, test2) {
    ...
    histograms.emplace_back(histo::Histo<double>(
            data, histo::GenerateBreaksFromRangeAndBins<double>(0.0, 15.0, 5)));

...
AddGraphPeninsulasResult add_graph_peninsulas(
  ...
  auto g_diff = spatial_graph_difference(
      graphs[high_info_graph_index],
  ...
  SG::append_new_graph_points(
      g_diff_points_map_pair.first,
      g_diff_points_map_pair.second,
      mergePoints,
      idMap);
  ...
  auto num_of_components = boost::connected_components(g_diff,
      boost::make_assoc_property_map(components_map));

  std::vector<size_t> touch_extended_graph_count(num_of_components);
  std::vector< std::vector<IdWithGraphDescriptor>> touching_descriptors(num_of_components);
  for(const auto & comp : components_map) {
    ...
    auto closest_descriptors = closest_existing_descriptors_by_graph(
        closeIdList, idMap);
    ...
    // if is_edge, the resulting graph will add a node and split the existing edge.
    // if is_vertex, no vertex needs to be added.
  ...
  for (unsigned int comp_graph_index = 0; comp_graph_index < num_of_components;
      ++comp_graph_index) {
    // Only analyze isolated components and peninsulas
    if(touch_extended_graph_count[comp_graph_index] > 1) continue;
    ...
    if(touching_descriptors[comp_graph_index].size() == 1) {
      const auto & touching_extended_descriptor =
        touching_descriptors[comp_graph_index][extended_graph_index].descriptor;
      const auto & touching_diff_descriptor =
        touching_descriptors[comp_graph_index][diff_graph_index].descriptor;
      if(touching_extended_descriptor.is_vertex) {
        // if the touching point is a vertex in the low info graph
        // store it to avoid adding it twice when iterating the component graph
        // XXX: Here we assume that the touching point is always a vertex in the diff graph
        // This is correct due to the spatial_graph_difference implementation.
        ...
        // if the touching point is an edge in the low graph, we have to split that
        // edge in the result graph when adding the vertex
        splitEdge = SG::split_edge(touching_extended_descriptor.edge_points_index,
            touching_extended_descriptor.edge_d, result_sg);
        already_added_vertex = splitEdge.vertex_descriptor_added;
      } else {
        std::runtime_error("Impossible, extended_graph descriptor has to exist at the touch point");
      }
    }
    ...
    // Add/Copy the component graph into the resulting grap
    {
      ...
      for(; ei != ei_end; ++ei) {
        ...
        const auto & touching_diff_descriptor =
          touching_descriptors[comp_graph_index][diff_graph_index].descriptor;
        // Note that the touching point of the diff is always a vertex, because
        // implementation of spatial_graph_difference
        const auto &diff_vertex_descriptor = touching_diff_descriptor.vertex_d;
        GraphType::vertex_descriptor result_source;
        GraphType::vertex_descriptor result_target;
        if(source != diff_vertex_descriptor) {
          result_source = boost::add_vertex(source_spatial_node, result_sg);
        } else {
          result_source = already_added_vertex;
        }
        if(target != diff_vertex_descriptor) {
          result_target = boost::add_vertex(target_spatial_node, result_sg);
        } else {
          result_target = already_added_vertex;
        }
        boost::add_edge(result_source, result_target, component_edge, result_sg);
      }
    } // end copying
  }

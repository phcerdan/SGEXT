...
struct SpatialGraphDifferenceVisitor : public boost::default_dfs_visitor {
  ...
  SpatialGraphDifferenceVisitor(
      ...
      vtkOctreePointLocator *octree, double &radius, ColorMap &color_map,
      VertexMap &vertex_map, bool &verbose)
  ...
  void discover_vertex(vertex_descriptor u, const SpatialGraph &input_sg) {
    ...
    // If it exists, it might still be added if an edge needs it when processing
    // edges
    if(!sg_vertex_exists) {
   ...
   *  Check if there is an edge in substraend graph between the same positions.
  ...
  void examine_edge(edge_descriptor e, const SpatialGraph &input_sg) {
    ...
    // Add both nodes if the edge has to be added. The edge needs to be added
    // if:
    // - Any of source/target DOES NOT exist in substraend graph. DONE
    // - Both source/target exist in substraend, but the edge between them does
    // not.
    std::array<bool, 2> input_nodes_exist_in_result_graph = {false, false};
    std::array<bool, 2> input_nodes_exist_in_substraend_graph = {false, false};
    for(unsigned int source_target_index = 0;
        source_target_index < input_nodes_sorted.size();
        ++source_target_index) {
      const vertex_descriptor &u = input_nodes_sorted[source_target_index];
      // Check if current node exist in the result graph already.
      auto vertex_search = m_vertex_map.find(u);
      if(vertex_search != m_vertex_map.end()) {
    ...
    auto closest_desc_source =
        get_closest_existing_descriptors(input_sg[input_nodes_sorted[0]].pos);
    auto closest_desc_target =
        get_closest_existing_descriptors(input_sg[input_nodes_sorted[1]].pos);
    ...
    if(source_and_target_are_not_in_result_graph) {
      if(closest_desc_source[substraend_index].descriptor.is_vertex &&
        ...
        auto any_edge_exist_minuend =
            boost::edge(input_nodes_sorted[0], input_nodes_sorted[1], input_sg);
        if(any_edge_exist_minuend.second) {
          const auto &substraend_source = closest_desc_source[substraend_index];
          const auto &substraend_target = closest_desc_target[substraend_index];
          ...
          if(!any_edge_exist_substraend.second) {
            ...
            result_nodes[0] =
                boost::add_vertex(input_sg[input_nodes_sorted[0]], m_result_sg);
            result_nodes[1] =
                boost::add_vertex(input_sg[input_nodes_sorted[1]], m_result_sg);
            m_vertex_map.emplace(input_nodes_sorted[0], result_nodes[0]);
            m_vertex_map.emplace(input_nodes_sorted[1], result_nodes[1]);
            boost::add_edge(result_nodes[0], result_nodes[1], input_sg[e],
                            m_result_sg);
            if(m_verbose) {
              std::cout << "ADD BOTH VERTEX: " << input_nodes_sorted[0]
                        << " and " << input_nodes_sorted[1] << std::endl;
    ...
    if(source_and_target_are_not_in_result_graph) return;

    // From here, source_or_target_are_in_result_graph is assumed to be true.
    ...
    if(only_one_source_or_target_is_in_result_graph) {
      ...
      if(!substraend_edge_exists) {
        ...
        auto find_result =
            std::find(std::begin(input_nodes_exist_in_result_graph),
                      std::end(input_nodes_exist_in_result_graph), false);
        assert(find_result != std::end(input_nodes_exist_in_result_graph));
        auto index_of_non_existant_vertex = std::distance(
            std::begin(input_nodes_exist_in_result_graph), find_result);
        auto non_existant_vertex =
            input_nodes_sorted[index_of_non_existant_vertex];

        result_nodes[index_of_non_existant_vertex] =
            boost::add_vertex(input_sg[non_existant_vertex], m_result_sg);
        m_vertex_map.emplace(non_existant_vertex,
                             result_nodes[index_of_non_existant_vertex]);
        if(m_verbose) {
          std::cout << "VERTEX ADDED in examine_edge: " << non_existant_vertex
                    << std::endl;
        }
      }
    }
    ...
    // Add edge
    {
      ...
      if(!any_edge_exist_result.second) {
        ...
        if(substraend_source.exist && substraend_source.descriptor.is_vertex) {
          if(substraend_target.exist &&
             substraend_target.descriptor.is_vertex) {
            // Both are also vertex in substraend graph,
            // add edge only if there is no edge in substraend graph.
            auto any_edge_exist_substraend = boost::edge(
                substraend_source.descriptor.vertex_d,
                substraend_target.descriptor.vertex_d, m_substraend_sg);
            if(!any_edge_exist_substraend.second) {
              if(m_verbose) {
                std::cout << "ADD_EDGE (both vertex exists in substraend, but "
                             "edge doesn't): ";
                std::cout << result_nodes[0] << ", " << result_nodes[1]
                          << std::endl;
              }
        ...
        if(substraend_source.exist && substraend_source.descriptor.is_edge) {
          if(m_verbose) std::cout << "Substraend Source: is_edge" << std::endl;
        }

        if(!substraend_source.exist || !substraend_target.exist) {
          if(m_verbose) {
            std::cout << "ADD_EDGE (any of the vertex does not exist in "
                         "substraend): ";
            std::cout << result_nodes[0] << ", " << result_nodes[1]
                      << std::endl;
          }
...
private:
  std::vector<IdWithGraphDescriptor> get_closest_existing_descriptors(
  ...
  add_new_vertex_if_does_not_exist_in_substraend_graph(
    ...
    if(m_verbose && exist_in_substraend_graph) {
      std::cout
          << "point_exist_in_substraend_graph: graph descriptor does exist"
          << std::endl;
    } else {
      std::cout
          << "point_exist_in_substraend_graph: graph descriptor does NOT exist"
          << std::endl;
    }
    if(!exist_in_substraend_graph) {
      sg_vertex_descriptor = boost::add_vertex(input_spatial_node, m_result_sg);
      added = true;
      if(m_verbose) std::cout << "VERTEX ADDED " << u << std::endl;

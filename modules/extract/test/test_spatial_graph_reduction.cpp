 ...
 * o-o
 */
struct sg_square : public SpatialGraphBaseFixture, ::testing::Test {
  void SetUp() override {
 ...
 * o
 */
struct sg_square_plus_one : public SpatialGraphBaseFixture, ::testing::Test {
  void SetUp() override {
    using boost::add_edge;
    this->g = GraphType(5);
    g[0].pos = {{0, 0, 0}};
...
struct sg_one_edge : public SpatialGraphBaseFixture {
  sg_one_edge() {
    ...
    SG::SpatialEdge se01;
    se01.edge_points.insert(std::end(se01.edge_points), {n2, n1, p0, s1, s2});
    add_edge(0, 1, se01, g);
  }
};
 ...
 *     o
 */
struct sg_easy : public SpatialGraphBaseFixture {
  sg_easy() {
    using boost::add_edge;
    this->g = GraphType(4);
    // Add edge with an associated SpatialEdge at construction.
...
struct object_graph : public ::testing::Test {
  ...
  Object obj;
  using vertex_iterator = typename boost::graph_traits<Object>::vertex_iterator;
  ...
  void print_edges() {
    ...
    for(; ei != ei_end; ++ei) {
      std::cout << boost::source(*ei, obj) << "---" << boost::target(*ei, obj)
                << std::endl;
    }
  }
};
...
// Helpers
SG::PointContainer all_vertex_positions(
    const SpatialGraphBaseFixture::GraphType &lhs_g) {
...
SG::PointContainer all_edge_points(
    const SpatialGraphBaseFixture::GraphType &lhs_g) {
...
TEST_F(three_connected_nodes, remove_extra_edges) {
  ...
  for(; vi != vi_end; ++vi) {
    ...
    if(degree == 1) count1degrees++;
    if(degree == 2) count2degrees++;
    if(degree == 3) count3degrees++;
  }
...
TEST_F(three_connected_nodes_with_self_loop, remove_extra_edges) {
  ...
  EXPECT_EQ(num_edges(sg),
            17);  // Haven't really thought about his, but should be ok.
  bool any_edge_removed = remove_extra_edges(sg);
...
TEST_F(rare_trio, remove_extra_edges) {
  ...
  auto nodes_merged =
      SG::merge_three_connected_nodes(reduced_g, !removeNodesInPlace);
  ...
  SG::print_degrees(reduced_g);
  SG::print_spatial_edges(reduced_g);
  auto nodes_merged_and_deleted = SG::merge_three_connected_nodes(
      reduced_g_delete_nodes, removeNodesInPlace);
  std::cout << nodes_merged_and_deleted
            << " nodes were merged. Those nodes were deletes form the graph."
            << std::endl;
...
TEST_F(buggy_structure, remove_extra_edges) {
  ...
  for(auto edge_pair = boost::edges(reduced_g);
      edge_pair.first != edge_pair.second; ++edge_pair.first) {
    const auto &eps = reduced_g[*edge_pair.first].edge_points;
    all_edge_points.insert(std::end(all_edge_points), std::begin(eps),
                           std::end(eps));
  }
  ...
  std::cout << nodes_merged
            << " nodes were merged. Those nodes have now degree 0" << std::endl;

...
size_t merge_three_connected_nodes(GraphType &sg, bool inPlace) {
  ...
  using adjacency_iterator = boost::graph_traits<GraphType>::adjacency_iterator;
  ...
  for(; vi != vi_end; ++vi) {
    ...
    if(degree_current == 3) {
      ...
      {
        auto already_selected_to_delete =
            std::find_if(std::begin(nodes_to_remove), std::end(nodes_to_remove),
                         [&vi](const VertexPair &p) { return p.first == *vi; });
        if(already_selected_to_delete != std::end(nodes_to_remove)) continue;
      }
        ...
        if(other_neighbor_it != neighbor_end_it) {
          for(; other_neighbor_it != neighbor_end_it; ++other_neighbor_it) {
            ...
            if(edge_exist_pair.second) {
              ...
              // Because removing edges causes invalidation of
              // adjacency_iterator
      ...
      for(auto &nv : neighbors_connected) {
        ...
        if(degree_first != 3 || degree_second != 3) continue;
        ...
        // If there are more than one edge connecting the node trio, abort.
        {
          ...
          for(auto it = std::begin(tri_nodes);
              it != std::end(tri_nodes) && !node_trio_has_parallel_edges;
              ++it) {
            auto it_other = it;
          ...
          if(node_trio_has_parallel_edges) continue;
        }
        ...
        if(!(edge_neighbors.edge_points.empty() &&
             edge_first.edge_points.empty() && edge_second.edge_points.empty()))
          continue;
  ...
  for(auto &edge_triple : edges_to_remove) {
    boost::remove_edge(std::get<0>(edge_triple), std::get<1>(edge_triple), sg);
    boost::remove_edge(std::get<0>(edge_triple), std::get<2>(edge_triple), sg);
    boost::remove_edge(std::get<1>(edge_triple), std::get<2>(edge_triple), sg);
  }
  ...
  for(auto &nodes : nodes_to_remove) {
    ...
    for(auto ei = edges.first; ei != edges.second; ++ei) {
      ...
      SG::insert_unique_edge_point_with_distance_order(spatial_edge.edge_points,
                                                       sn_to_remove.pos);
...
size_t merge_two_three_connected_nodes(GraphType &sg, bool inPlace) {
    ...
    if(degree_current == 3) {
      // Check that current node is not already selected to remove:
      {
        auto already_selected_to_delete = std::find_if(
            ...
            [&vi](const VertexTriple &p) { return std::get<0>(p) == *vi; });
        if(already_selected_to_delete != std::end(nodes_to_remove)) continue;
      }
      adjacency_iterator neighbor_it, neighbor_end_it;
      std::tie(neighbor_it, neighbor_end_it) =
      ...
      for(; neighbor_it != neighbor_end_it; ++neighbor_it) {
        const auto degree_neighbor = boost::out_degree(*neighbor_it, sg);
      ...
      if(three_vertices_connected.size() == 0) continue;
      if(three_vertices_connected.size() > 1) {
        std::cout
            << "WARNING: In merge_two_three_connected_nodes "
               "there are two vertices with degree 3 connected with parallel "
               "edges "
               "with no edge points. "
               "Extremely rare? Holes? Logic not handled. No action taken."
            << std::endl;
        continue;
      }
      ...
      // Check if the edge points of the three vertices are able to be merged
      //
      ...
      for(auto ei = out_edges.first; ei != out_edges.second; ++ei) {
        auto target = boost::target(*ei, sg);
        if(target == connected_vertex_candidate_to_remove) continue;
        const auto &edge_points = sg[*ei].edge_points;
        std::vector<double> distances_to_candidate;
        const auto ep_size = edge_points.size();
        if(ep_size == 0) continue;
        if(ep_size == 1) {
          distances_to_candidate.emplace_back(
              ArrayUtilities::distance(edge_points[0], sn_candidate.pos));
        }
        if(ep_size > 1) {
          distances_to_candidate.emplace_back(ArrayUtilities::distance(
              edge_points[ep_size - 1], sn_candidate.pos));
        }
        // Check that any of the outgoing edge points (only check begin and end)
        // touch (26_9 -- compute distance) the three connected vertex.
        auto min_it = std::min_element(std::begin(distances_to_candidate),
        ...
        if(min_dist >
          ...
          nodes_to_remove.emplace_back(connected_vertex_candidate_to_remove,
                                       *vi, target);
  ...
  for(auto &node_triple : nodes_to_remove) {
    ...
    auto &node_which_edge_does_not_require_add_point = std::get<2>(node_triple);
    auto edges = boost::out_edges(node_to_remove, sg);
    // Create an edge to the node to merge into.
    for(auto ei = edges.first; ei != edges.second; ++ei) {
      auto target = boost::target(*ei, sg);
      if(target == node_to_merge_into) {
        ...
        // std::cout << "Spatial Edge WITHOUT added point:" << std::endl;

...
size_t merge_two_three_connected_nodes(GraphType & sg, bool inPlace)
{
    ...
    // Third vertex descriptor is the node which edge just needs a change of target
        ...
        if (degree_current == 3){
            ...
            for(; neighbor_it != neighbor_end_it; ++neighbor_it) {
                ...
                if(degree_neighbor == 3) { // current degree has also degree 3
                    const auto & spatial_edge = sg[boost::edge(*vi, *neighbor_it, sg).first];
                    // If the edge_points are not empty, abort merge
            ...
            if(three_vertices_connected.size() > 1) {
                std::cout << "WARNING: In merge_two_three_connected_nodes "
                    "there are two vertices with degree 3 connected with parallel edges "
                    "with no edge points. "
                    "Extremely rare? Holes? Logic not handled. No action taken." << std::endl;
                continue;
            }
            const auto &connected_vertex_candidate_to_remove = three_vertices_connected[0];
            ...
            // std::vector<std::pair<vertex_descriptor, vertex_descriptor>> outgoing_edges;
            const auto out_edges = boost::out_edges(connected_vertex_candidate_to_remove, sg);
            for (auto ei = out_edges.first; ei != out_edges.second; ++ei) {
                ...
                if(ep_size == 1) {
                    distances_to_candidate.emplace_back(ArrayUtilities::distance(edge_points[0], sn_candidate.pos));
                }
                if(ep_size > 1) {
                    distances_to_candidate.emplace_back(ArrayUtilities::distance(edge_points[ep_size - 1], sn_candidate.pos));
                }
                ...
                if(min_dist > sqrt(3.0) + 2.0 * std::numeric_limits<double>::epsilon()) {
                    ...
                    nodes_to_remove.emplace_back(connected_vertex_candidate_to_remove, *vi, target);
        ...
        for(auto ei = edges.first ; ei != edges.second; ++ei) {
            ...
            if(target == node_to_merge_into) {
                ...
                boost::add_edge(node_to_merge_into,
                        node_which_edge_does_not_require_add_point, spatial_edge, sg);
                // DEBUG REMOVE

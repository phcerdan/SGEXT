...
struct ReduceGraphVisitor : public boost::default_dfs_visitor {
  ...
  ReduceGraphVisitor(SpatialGraph &sg, ColorMap &color_map,
                     VertexMap &vertex_map, bool &is_not_loop, bool &verbose)
      : m_sg(sg),
...
public:
  ...
  void discover_vertex(vertex_descriptor u,
    ...
    if(degree != 2)  // starting or ending point.
    {
      ...
      if(!m_already_started) {  // Starting node
        ...
        if(!m_sg_edge.edge_points.empty()) m_sg_edge.edge_points.pop_back();
        ...
        if(!any_edge_exist.second) {
          ...
          // if(m_verbose)
          //   std::cout << "Added new edge: (" << m_source << "," << u << ") "
          //   << m_sg[added.first] << std::endl;
        } else {
          // Iterate over all parallel edges and checks if any of them is equal
          // to current edge. If not, add it.
          ...
          for(; ei != ei_end && !current_edge_already_exist; ++ei)
            if(boost::target(*ei, m_sg) == sg_vertex_descriptor) {
              ...
              auto &parallel_edge_points = m_sg[parallel_edge].edge_points;
              if(parallel_edge_points.size() == m_sg_edge.edge_points.size()) {
                auto sorted_current_edge_points = m_sg_edge.edge_points;
                std::sort(std::begin(sorted_current_edge_points),
                          std::end(sorted_current_edge_points));
                auto sorted_parallel_edge_points = parallel_edge_points;
                std::sort(std::begin(sorted_parallel_edge_points),
                          std::end(sorted_parallel_edge_points));
                if(sorted_current_edge_points == sorted_parallel_edge_points) {
                  // Match, edge exist
          ...
          if(!current_edge_already_exist) {
            boost::add_edge(m_sg_source, sg_vertex_descriptor, m_sg_edge, m_sg);
            // if(m_verbose)
            //   std::cout << "Added parallel edge  (" << m_source << "," << u
            //   << ") " << m_sg[added.first] << std::endl;
          }
        }  // end add parallel edge
      }

      if(!sg_vertex_exists) m_vertex_map.emplace(u, sg_vertex_descriptor);
  ...
  void tree_edge(edge_descriptor e, const SpatialGraph &input_sg) {
    ...
    // inmediately.
    // We can check that if the source of e is the start vertex,(beggining of a
    // branch) clear the edge_points.
    auto source = boost::source(e, input_sg);
    if(source == m_source) m_sg_edge.edge_points.clear();
  ...
  void back_edge(edge_descriptor e, const SpatialGraph &input_sg) {
    ...
    if(!m_is_not_loop && target == m_source &&
      ...
      if(m_verbose) std::cout << "Loop: " << target << std::endl;
      m_is_not_loop = false;
  ...
  void finish_vertex(vertex_descriptor u, const SpatialGraph &input_sg) {
    ...
    if(boost::out_degree(u, input_sg) > 2) {
      ...
      // Restore white
      if(has_white_neighbors) m_color_map[u] = Color::white();
    }
  }
};

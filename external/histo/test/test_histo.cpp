...
TEST(HistoConstructor, withBreaksWithGenerator) {
  ...
  Histo<double> h(data, histo::GenerateBreaksFromRangeAndBins<double>(
                            0.0, 20.0, input_bins));
...
Histo<double> HistoIntInputBreaks::h(
    data, histo::GenerateBreaksFromRangeAndBins<double>(-2, 2, 2));
...
/**
 * @brief You can modify counts manually, or with methods Increase, Decrease,
 * SetCount. The latter provides security checks, throwing exepction if exceed
 * limits (big, or less than zero).
 */
...
/**
 * @brief Test the constructor with just double data and Scott method to
 * generate breaks.
 */
...
TEST_F(HistoDoubleScott, IndexFromValueWorks) {
  ASSERT_THROW(h.IndexFromValue(1.0 - numeric_limits<double>::epsilon()),
               histo_error);
  EXPECT_EQ(0, h.IndexFromValue(1.0));
  EXPECT_EQ(
      0, h.IndexFromValue(h.breaks[1] - 2 * numeric_limits<double>::epsilon()))
      << h.breaks[1];
  EXPECT_EQ(0, h.IndexFromValue(h.breaks[1])) << h.breaks[1];
  ASSERT_THROW(h.IndexFromValue(10), histo_error);
}
/**
 * @brief Test the constructor with long double, input_range, and specific
 * method to generate breaks.
 */
...
Histo<long double> HistoLongDoubleInputRange::h(data, std::make_pair(-1.0, 1.0),
                                                breaks_method::Scott);
...
TEST(HistoLotsOfDataInputRange, BalanceBreaksWorks) {
  ...
  Histo<long double, unsigned long long> h(data, input_pair,
                                           breaks_method::Scott);

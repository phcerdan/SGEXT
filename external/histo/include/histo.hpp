/**
@file histo.hpp
Histogram in C++11, 1D, simple header-only, inspired by R, calculate and
optimize breaks automatically. Accepts different precissions. Copyright (C) 2015
Pablo Hernandez. github/phcerdan/histo-header.
...
template <typename PRECI = double>
std::vector<PRECI> GenerateBreaksFromRangeAndBins(
    const std::pair<PRECI, PRECI> &range_low_upper,
...
struct Histo {
  ...
  unsigned long int IndexFromValue(const TData &value) const {
    ...
    // typename std::vector<T>::iterator low = std::lower_bound(breaks.begin(),
    // breaks.end(), value);
    unsigned long int lo{0}, hi{bins},
        newb;  // include right border in the last bin.
    if(value >= breaks[lo] &&
       (value < breaks[hi] || histo::isequalthan<PRECI>(value, breaks[hi]))) {
      while(hi - lo >= 2) {
        newb = (hi + lo) / 2;
  ...
  void Increase(const unsigned long int &index) {
    if(counts[index] == std::numeric_limits<PRECI_INTEGER>::max())
      throw histo_error(
          "Increase has exceded PRECI_INTEGER."
          " Index: " +
          std::to_string(index) + " Value: " + std::to_string(counts[index]));
    counts[index]++;
  };
  ...
  void Decrease(const unsigned long int &index) {
    if(counts[index] <= 0)
      throw histo_error(
          "Decrease has reached negative value."
          " Index: " +
          std::to_string(index) + " Value: " + std::to_string(counts[index]));
    counts[index]--;
  };
  ...
  void SetCount(const unsigned long int &index, const PRECI_INTEGER &v) {
    if(index > bins)
      throw histo_error(
          "Index is out of bounds in SetCount"
          " Index: " +
          std::to_string(index) + " Max Bins: " + std::to_string(bins));
    counts[index] = v;
  };
...
protected:
  ...
  std::vector<PRECI> &CalculateBreaks(const std::vector<TData> &data,
    ...
    switch(method) {
      ...
      default:
        throw histo_error(
            "CalculateBreaks: No Valid Method selected to calculate breaks.");
    }
  };
  ...
  bool BalanceBreaksWithRange(std::vector<PRECI> &input_breaks,
    ...
    if(!CheckBreaksAreEquidistant(input_breaks)) {
      ...
      throw histo_error(
          "BalanceBreaksWithRange cannot be applied in NON Equidistant breaks");
    }
  ...
  std::vector<PRECI> &ScottMethod(const std::vector<TData> &data,
    ...
    // std::cout<< "bins is: " << bins <<" width is: "<< width << " sigma is: "
    ...
    // <<std::endl; std::cout << "first break" << breaks[0] << " Second Break"
 ...
 */
template <typename PRECI = double, typename PRECI_INTEGER = unsigned long int>
Histo<PRECI, PRECI> NormalizeByArea(
    const Histo<PRECI, PRECI_INTEGER> &input_histo) {

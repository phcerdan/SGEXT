...
min_max(TImagePointer &img) {
    std::pair<typename TImagePointer::ObjectType::PixelType,
              typename TImagePointer::ObjectType::PixelType>
            min_max_values;
    auto min_max_filter = itk::MinimumMaximumImageFilter<
            typename TImagePointer::ObjectType>::New();
    min_max_filter->SetInput(img);
    ...
    py::class_<TImagePointer>(m, typestr.c_str())
            ...
            .def(
                    ...
                    [](TImagePointer &img,
                       py::array_t<int, py::array::c_style |
                                                py::array::forcecast> index,
                       py::array_t<int, py::array::c_style |
                                                py::array::forcecast> size) {
            ...
            .def("set_direction",
                 [](TImagePointer &img,
                    py::array_t<double,
                     ...
                     if (buf.ndim != 2) {
                         throw std::runtime_error("Number of dimensions must "
                                                  "be 2 (square matrix). "
                                                  "But it is: " +
                                                  std::to_string(buf.ndim));
                     }
                     if (buf.shape[0] != img->ImageDimension ||
                         ...
                         throw std::runtime_error(
                                 "Shape should be Dimension x Dimension, but "
                                 "shape = [ " +
                                 std::to_string(buf.shape[0]) + ", " +
                     ...
                     typename TImagePointer::ObjectType::DirectionType
                             itk_direction(vnl_matrix);
            ...
            .def("get_pixel",
                 [](TImagePointer &img,
                    py::array_t<int, py::array::c_style | py::array::forcecast>
                            index) {
                     using IndexType =
                             typename TImagePointer::ObjectType::IndexType;
                     IndexType itk_index;
                     const auto *data_index = static_cast<int *>(
                             np_array_ptr_after_check_dim_and_shape<int>(
                                     index));
                     for (size_t i = 0;
                          i < TImagePointer::ObjectType::ImageDimension; i++) {
            ...
            .def("set_pixel",
                 [](TImagePointer &img,
                    py::array_t<int, py::array::c_style | py::array::forcecast>
                            index,
                    const typename TImagePointer::ObjectType::PixelType
                            &value) {
                     using IndexType =
                             typename TImagePointer::ObjectType::IndexType;
                     IndexType itk_index;
            ...
            .def("transform_physical_point_to_index",
                 [](TImagePointer &img,
                    py::array_t<double,
                                py::array::c_style | py::array::forcecast>
                            physical_point) {
                     auto *data = static_cast<double *>(
                             np_array_ptr_after_check_dim_and_shape<double>(
                                     physical_point));
                     using PointType =
                             typename TImagePointer::ObjectType::PointType;
                     using IndexType =
                             typename TImagePointer::ObjectType::IndexType;
                     PointType itk_point;
                     for (size_t i = 0;
                     ...
                     return py::array(TImagePointer::ObjectType::ImageDimension,
                                      itk_index.data());
                 })

            .def("transform_physical_point_to_continuous_index",
                 [](TImagePointer &img,
                    py::array_t<double,
                                py::array::c_style | py::array::forcecast>
                            physical_point) {
                     ...
                     using ContinuousIndexType = itk::ContinuousIndex<
                             double, TImagePointer::ObjectType::ImageDimension>;
                     ContinuousIndexType itk_cindex;
                     img->TransformPhysicalPointToContinuousIndex(itk_point,
                                                                  itk_cindex);
                     return py::array(TImagePointer::ObjectType::ImageDimension,
                                      itk_cindex.GetDataPointer());
                 })

            .def("transform_index_to_physical_point",
                 [](TImagePointer &img,
                    py::array_t<int, py::array::c_style | py::array::forcecast>
                            index) {
                     using IndexType =
                     ...
                     return py::array(TImagePointer::ObjectType::ImageDimension,
                                      itk_point.GetDataPointer());
                 })

            // Compute min_max_values with MinimumMaximumImageFilter
            .def("min_max", [](TImagePointer &img) {
                 return min_max(img);
                 })

            // Follow numpy: CONTIG can be 'C' or 'F'
            // numpy default is F <- this is a pain coming from C data.
            // python ecosystem assume a F layout, so we return it as the
            // default.
            .def(
                    "to_pyarray",
                    [](const TImagePointer &img,
